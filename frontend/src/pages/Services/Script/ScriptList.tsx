...
// Pastikan path import types ini sesuai lokasi file types.ts Anda
import { Script } from "../types"; 

// Kita gunakan icon mata lokal (sama seperti di ProductList)
...
export default function ScriptList() {
  ...
  // 3. FETCH SCRIPTS (Gunakan API.scripts)
  ...
  // HANDLER DELETE (Gunakan API.scripts)
  const confirmDeleteScript = async () => {
    if (!deleteTargetId) return;
    setIsDeleting(true);
    
    // UBAH DISINI: API.scripts.detail
      ...
      <div className="flex flex-col gap-5 md:gap-7 2xl:gap-10 mt-5">
        <div className="rounded-sm border border-stroke bg-white px-5 py-4 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            {/* Tombol Tambah (Arahkan ke route script) */}
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark overflow-hidden">
                  ...
                  scripts.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-meta-4/50 transition-colors">
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark text-center">
                         {item.is_breaking ? (
                            <span className="inline-flex rounded-full bg-red-100 text-red-800 px-3 py-1 text-xs font-medium">Breaking</span>
                         ) : (
                            <span className="inline-flex rounded-full bg-blue-100 text-blue-800 px-3 py-1 text-xs font-medium">Standard</span>
                         )}
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
          ...
          <div className="py-4 px-6 border-t border-stroke dark:border-strokedark flex justify-between items-center bg-gray-50 dark:bg-meta-4/20">
             ...
             <div className="flex gap-2">
                <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1 rounded border border-stroke bg-white text-sm hover:bg-gray-50 disabled:opacity-50 dark:border-strokedark dark:bg-meta-4">Prev</button>
                <span className="px-3 py-1 text-sm font-medium bg-white border border-stroke rounded dark:bg-meta-4 dark:border-strokedark">{page}</span>
                <button disabled={scripts.length < limit} onClick={() => setPage(p => p + 1)} className="px-3 py-1 rounded border border-stroke bg-white text-sm hover:bg-gray-50 disabled:opacity-50 dark:border-strokedark dark:bg-meta-4">Next</button>
             </div>
          </div>
        </div>
      </div>
